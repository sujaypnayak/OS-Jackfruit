 ...
 *   - pid, container ID, soft/hard limits
 *   - soft_warned: set after the first soft-limit warning so we
 *     only log it once per entry
 ...
 *
 * We use a mutex rather than a spinlock because the timer callback
 * runs in a softirq-deferred context (tasklet / timer wheel) on
 * modern kernels but the ioctl path may sleep during copy_from_user.
 * A mutex is safe for both paths here: the timer callback does not
 * actually sleep inside the critical section, and we never call the
 * timer path from hard-IRQ context.  Using a mutex also keeps the
 * locking model simple and easy to reason about in the README.
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
     ...
     *
     * Walk the list under the lock.  Use list_for_each_entry_safe so
     * we can delete nodes while iterating without use-after-free.
     *
     * For each entry:
     *   1. Call get_rss_bytes(). If -1, the process has exited —
     *      remove and free the entry.
     *   2. If rss >= hard limit: kill, remove, free.
     *   3. Else if rss >= soft limit and !soft_warned: log warning,
     *      set soft_warned = 1 (do NOT remove — keep tracking).
     * ============================================================== */
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        /* Soft limit: warn once, keep tracking */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
     ...
     *
     * Search by both PID and container_id for precision.
...
static void __exit monitor_exit(void)
{
     ...
     *
     * Walk the full list and kfree every node.  The timer is already
     * stopped by del_timer_sync above so no concurrent access is
     * possible here.
     * ============================================================== */
    {
        struct monitored_entry *entry, *tmp;
